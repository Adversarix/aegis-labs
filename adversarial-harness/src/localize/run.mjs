...
import { makeMediator } from "./gate.mjs";
...
async function runOne(backend, target) {
  ...
  // hypothesize -> CONFIRM (static sink over the real source; §8)
  const readFile = (p) => readFileSync(join(repoDir, p), "utf8");
  const findings = toFindings(result, { trajectory_ref: log }).map((f) => confirmSource(f, readFile));
  ...
  return { backend: backend.id, model: backend.model, target: target.id,
    submitted: result.ranked_files, confirmed: confirmed_files, denied, calls: result.calls, seconds,
...
for (const backend of BACKENDS) {
  for (const target of tasks.targets) {
    ...
    try {
      ...
      rows.push(r);
      console.log(`submitted=${JSON.stringify(r.submitted)} confirmed=${JSON.stringify(r.confirmed)} ` +
